...
class ExpireSKU extends Inventory{
    ...
    @Override 
    public double calculateLiquidationFee(double inventoryValue){
    this.baseFee= super.calculateLiquidationFee(inventoryValue);
    if(!expireSKU){return this.baseFee;}
    return this.baseFee + (inventoryValue*0.05);
    }}
...
if(iv instanceof ExpireSKU){
    baseFee = ((ExpireSKU) iv).baseFee;
}

       System.out.printf("SKU: %s | Salable Qty: %d | Storage Fee: %2f  | Inventory Value: %.2f\n | Liquidation Fee: %.2f\n | Base Fee: %.2f\n", iv.sku,iv.salableQty,iv.storageFee,inventoryValue,liquidationSubtotal,baseFee);}
